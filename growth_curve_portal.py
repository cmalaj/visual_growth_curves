...
# -----------------------------------------------
# Export selected plot interactively by dropdown
# -----------------------------------------------
st.markdown("---")
st.header("📤 Export a Specific Plate's Plot")

if export_options:
    selected_plate = st.selectbox("🧪 Select Plate to Export", export_options)

    if selected_plate in export_figures:
        fig = export_figures[selected_plate]
        st.plotly_chart(fig, use_container_width=True)

        dpi_ts = st.number_input("📐 Time-Series Export DPI", 72, 600, 300, 10, key="ts_dpi_dropdown")
        scale = dpi_ts / 72
        ...
        ts_buf = io.BytesIO()
        fig.write_image(ts_buf, format="png", scale=scale)
        st.download_button("📥 Download Time-Series PNG", ts_buf.getvalue(), file_name=f"{selected_plate}_timeseries.png")

        ts_buf.seek(0)
        ts_img = Image.open(ts_buf)
        tiff_buf = io.BytesIO()
        ts_img.save(tiff_buf, format="TIFF")
        st.download_button("📥 Download Time-Series TIFF", tiff_buf.getvalue(), file_name=f"{selected_plate}_timeseries.tiff")

    if selected_plate in export_heatmaps:
        fig_hm = export_heatmaps[selected_plate]
        st.pyplot(fig_hm)

        dpi_hm = st.number_input("📐 Heatmap Export DPI", 72, 600, 300, 10, key="hm_dpi_dropdown")
        hm_png_buf = io.BytesIO()
        fig_hm.savefig(hm_png_buf, format="png", dpi=dpi_hm, bbox_inches="tight")
        st.download_button("📥 Download Heatmap PNG", hm_png_buf.getvalue(), file_name=f"{selected_plate}_heatmap.png")

        hm_png_buf.seek(0)
        hm_img = Image.open(hm_png_buf)
        hm_tiff_buf = io.BytesIO()
        hm_img.save(hm_tiff_buf, format="TIFF")
        st.download_button("📥 Download Heatmap TIFF", hm_tiff_buf.getvalue(), file_name=f"{selected_plate}_heatmap.tiff")

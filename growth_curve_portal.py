...
# Load optional metadata layout file
metadata_file = st.file_uploader("Upload optional layout metadata file", type="txt", key="metadata_file")
metadata_labels = {}

if metadata_file:
    try:
        metadata_df = pd.read_csv(metadata_file, sep="\t|,", engine="python", header=None, names=["Well", "Label"])
        for _, row in metadata_df.iterrows():
            well, label = row["Well"], row["Label"]
            if isinstance(well, str) and isinstance(label, str):
                metadata_labels[well.strip()] = label.strip()
